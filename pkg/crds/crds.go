...
const (
	// yamlDir directory holding CRDs yaml files.
	yamlDir = "yaml"
...
// Ensure looks for matching crd names insied the embedded directory 'yaml'
...
func getCRDs(crdNames []string) ([]*apiextv1.CustomResourceDefinition, error) {
	// create a map of crds that we want to return from this function.
	toReturn := []string{}
	for _, crdName := range crdNames {
		...
		if MigratedResources[crdName] {
			toReturn = append(toReturn, crdName)
		}
	}

	allCRDs, err := crdsFromDir(yamlDir)
	if err != nil {
		return nil, err
	}
...
func crdsFromDir(dirName string) (map[string]*apiextv1.CustomResourceDefinition, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read embedded dir '%s': %w", yamlDir, err)
	}
	...
	for _, dirEntry := range crdFiles {
		...
		for _, crdObj := range crdObjs {
			if crdObj.Kind != "CustomResourceDefinition" {
				// If the yaml was not a CRD skip it.
				continue
			}
