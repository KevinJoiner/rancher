...
type ProjectMonitorGraphGenericController struct {
	generic.ControllerInterface[*v3.ProjectMonitorGraph, *v3.ProjectMonitorGraphList]
}

func (c *ProjectMonitorGraphGenericController) OnChange(ctx context.Context, name string, sync ProjectMonitorGraphHandler) {
	c.ControllerInterface.OnChange(ctx, name, generic.ObjectHandler[*v3.ProjectMonitorGraph](sync))
}

func (c *ProjectMonitorGraphGenericController) OnRemove(ctx context.Context, name string, sync ProjectMonitorGraphHandler) {
	c.ControllerInterface.OnRemove(ctx, name, generic.ObjectHandler[*v3.ProjectMonitorGraph](sync))
}

func (c *ProjectMonitorGraphGenericController) Cache() ProjectMonitorGraphCache {
	return &ProjectMonitorGraphGenericCache{
		c.ControllerInterface.Cache(),
	}
}

type ProjectMonitorGraphGenericCache struct {
	generic.CacheInterface[*v3.ProjectMonitorGraph]
}

func (c ProjectMonitorGraphGenericCache) AddIndexer(indexName string, indexer ProjectMonitorGraphIndexer) {
	c.CacheInterface.AddIndexer(indexName, generic.Indexer[*v3.ProjectMonitorGraph](indexer))
}
